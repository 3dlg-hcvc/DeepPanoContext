...
class Bdb3DNet(nn.Module):
    ...
    def forward(self, x, size_cls, g_features, split):
        '''
        ...
        rel_pair_counts = torch.cat([torch.tensor([0], device=split.device), torch.cumsum(
        ...
        a_r_features_cat = torch.cat([ar_features, size_cls], 1)
        ...
        return {
            'bdb3d': {
                'size_reg': size, 'ori_reg': ori_reg, 'ori_cls': ori_cls,
                'dis_reg': centroid_reg, 'dis_cls': centroid_cls
            },
            'delta2d': offset,
            'ben_afeature': a_features,
            'ben_rfeature': r_features,
            'ben_arfeature': ar_features
        }
